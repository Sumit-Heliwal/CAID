...
def PTRC(request):
    title = "PTRC"
    if request.method == 'POST':
            code = int(request.POST.get('DIN_no'))
            if(PTRC_no_file.objects.filter(PTRC_no=code).exists()):
                data = PTRC_no_file.objects.filter(PTRC_no=code).values()
                a = True
...
def PTRC_no(request, code__no):
    title = "PTRC"
    if(PTRC_no_file.objects.filter(PTRC_no=code__no).values().exists()):
        data = PTRC_no_file.objects.filter(PTRC_no=code__no).values()
        a = True
        return render(request,'PTRC_search.html' ,{'title' : title , 'data' : data , 'a' : a })     
    else:
        errors = 'Invalid code'
