...
def personal_data(request):
    ...
    if request.method == 'POST':
            ...
            if(personal_file.objects.filter(code_no=code).exists()):
                ...
                a = True
                return render(request,'student.html' ,{'title' : title , 'data' : data , 'a' : a})     
            else:
                errors = 'Invalid code'
                a = False
                return render(request,'student.html' ,{'title' : title , 'errors': errors , 'a' : a})     
    return render( request, "student.html", { 'title' : title})
